...
#   - moves the foreground shape to post-curl step
...
is_within_circle <- function(x, y, d) {
  x <- normalise(x) - .5
  y <- normalise(y) - .5
...
create_layered_image <- function(data, shape_size, shape_contrast) {
  data |>
    dplyr::mutate(
      shape = is_within_circle(x_coord, y_coord, shape_size),
...
create_image_parameters <- function(seed) {
  ...
  list(
    ...
    layer_shape_contrast = sample_uniform(min = 0, max = 1),
...
if (FALSE) for (s in 141:160) curled(s)
