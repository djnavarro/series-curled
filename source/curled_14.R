...
#   - allows origin_x and origin_y to be top level parameters
#   - uses this to create regularly spaced horizontal stripes
#   - modifies message stem to list the system id
...
sample_uniform <- function(min, max, seed = NULL, n = 1) {
  if (!is.null(seed)) set.seed(seed)
...
is_within_v_band <- function(x, y, size, origin_x = .5, origin_y = .5) {
  x <- normalise(x) - origin_x
  return(abs(x) < (size / 2))
}

is_within_h_band <- function(x, y, size, origin_x = .5, origin_y = .5) {
  y <- normalise(y) - origin_y
  return(abs(y) < (size / 2))
}

filter_with_shape <- function(
    data,
    shape_type,
    shape_size,
    shape_contrast,
  ...
  if (shape_type == "v_band") is_within_shape <- is_within_v_band
  if (shape_type == "h_band") is_within_shape <- is_within_h_band
...
create_layered_image <- function(
  ...
  if (is.null(origin_x)) origin_x <- sample_uniform(min = 0, max = 1, n = n_shapes)
  if (is.null(origin_y)) origin_y <- sample_uniform(min = 0, max = 1, n = n_shapes)

  for(s in 1:n_shapes) {

    data <- data |> filter_with_shape(
      shape_type,
      shape_size,
      shape_contrast,
      origin_x[s],
...
create_image_parameters <- function(seed) {
  ...
  list(
    ...
    layer_shape_size = sample_uniform(min = .4, max = 1),
    layer_shape_contrast = sample_uniform(min = .05, max = .1),
    layer_shape_type = "h_band",
    layer_n_shapes = 7,
    layer_origin_x = NULL,
    layer_origin_y = seq(-.1, 1.1, .2),
...
curled <- function(seed) {
  ...
  message_stem <- paste0("system ", sys_id, ", seed ", seed, ":")
  ...
  image_data <- create_layered_image(
    ...
    origin_x = params$layer_origin_x,
    origin_y = params$layer_origin_y
  )
...
if (FALSE) for (s in 241:260) curled(s)
