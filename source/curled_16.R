# forked from 15, different base image mechanism
...
create_base_image <- function(seed) {
  ...
  # set up plot
  dat <- sample_data(seed, n = 100)
  plt <- polar_styled_plot(dat, grey.colors(4)) +
    ggplot2::geom_segment(data = dat) +
    ggplot2::geom_segment(data = dat |> dplyr::mutate(y1 = y1 - .2, y0 = y0 - .2)) +
    ggplot2::geom_segment(data = dat |> dplyr::mutate(y1 = y1 - .4, y0 = y0 - .4))
...
if (TRUE) for (s in 1600:1699) curled(s)
