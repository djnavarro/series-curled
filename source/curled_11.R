...
#   - uses multiple circles for the foreground filter
...
is_within_circle <- function(x, y, diameter, origin_x = .5, origin_y = .5) {
  x <- normalise(x) - origin_x
  y <- normalise(y) - origin_y
  return(x^2 + y^2 < (diameter / 2)^2)
}

filter_with_circle <- function(data, shape_size, shape_contrast) {
  data |>
    dplyr::mutate(
      shape = is_within_circle(
        x_coord,
        y_coord,
        shape_size,
...
create_layered_image <- function(data, shape_size, shape_contrast, n_shapes) {
  for(s in 1:n_shapes) {
    data <- filter_with_circle(data, shape_size, shape_contrast)
  }
...
create_image_parameters <- function(seed) {
  ...
  list(
    ...
    layer_shape_size = sample_uniform(min = .4, max = .8),
    layer_shape_contrast = sample_uniform(min = .1, max = .5),
    layer_n_shapes = sample_range(min = 5, max = 10),
...
if (FALSE) for (s in 161:180) curled(s)
